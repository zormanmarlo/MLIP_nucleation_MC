...
class TranslationMove(Move):
    ...
    def attempt_move(self, molecule_idx):
        ...
        # Check if this is Ca and track old proximity
        is_ca = self.system.molecule_type[molecule_idx] == 0
        if is_ca:
            has_close_ca_old, old_close_ca = self.system.check_ca_proximity(molecule_idx, 3.0)
        ...
        # Check new proximity before accept/reject
        if is_ca:
            has_close_ca_new, new_close_ca = self.system.check_ca_proximity(molecule_idx, 3.0)

        # Accept/reject with Metropolis criterion
        acc_prob = min(1, np.exp(np.clip((-(delta_energy + bias_energy) / self.system.kT), -500, 500)))
        accepted = np.random.rand() < acc_prob

        # Log if Ca is close to another Ca (either before or after)
        if is_ca and (has_close_ca_old or has_close_ca_new):
            if self.system.ca_logger:
                # Format distances
                old_dists = [f"{dist:.3f}" for _, dist in old_close_ca] if old_close_ca else []
                new_dists = [f"{dist:.3f}" for _, dist in new_close_ca] if new_close_ca else []
                old_str = ", ".join(old_dists) if old_dists else "none"
                new_str = ", ".join(new_dists) if new_dists else "none"

                self.system.ca_logger.warning(f"Step {self.system.current_step} | TranslationMove | Ca {molecule_idx}")
                self.system.ca_logger.warning(f"  Initial Ca-Ca distances (Å): {old_str}")
                self.system.ca_logger.warning(f"  Final Ca-Ca distances (Å):   {new_str}")
                self.system.ca_logger.warning(f"  Energy: {old_energy:.2f} → {new_energy:.2f} (ΔE={delta_energy:.2f}, bias={bias_energy:.2f})")
                self.system.ca_logger.warning(f"  Accepted: {accepted} (prob={acc_prob:.4f})")
                self.system.ca_logger.warning("")
    ...
    def attempt_move(self, molecule_idx):
        ...
        if is_ca and (has_close_ca_old or has_close_ca_new):
            if self.system.ca_logger:
                ...
                self.system.ca_logger.warning(f"Step {self.system.current_step} | SwapMove | Ca {molecule_idx}")
                self.system.ca_logger.warning(f"  Initial Ca-Ca distances (Å): {old_str}")
                self.system.ca_logger.warning(f"  Final Ca-Ca distances (Å):   {new_str}")
                self.system.ca_logger.warning(f"  Energy: {old_energy:.2f} → {new_energy:.2f} (ΔE={delta_energy:.2f}, bias={bias_energy:.2f})")
                self.system.ca_logger.warning(f"  Accepted: {accepted} (prob={acc_prob:.4f})")
...
class InOutAVBMCMove(Move):
    ...
    def attempt_move(self, anchor_idx):
        ...
        # Check if this is Ca and track old proximity
        is_ca = self.system.molecule_type[target_idx] == 0
        if is_ca:
            has_close_ca_old, old_close_ca = self.system.check_ca_proximity(target_idx, 3.0)
        ...
        # Check new proximity before accept/reject
        if is_ca:
            has_close_ca_new, new_close_ca = self.system.check_ca_proximity(target_idx, 3.0)
        ...
        if is_ca and (has_close_ca_old or has_close_ca_new):
            if self.system.ca_logger:
                ...
                self.system.ca_logger.warning(f"Step {self.system.current_step} | InOutAVBMCMove | Ca {target_idx}")
                self.system.ca_logger.warning(f"  Initial Ca-Ca distances (Å): {old_str}")
                self.system.ca_logger.warning(f"  Final Ca-Ca distances (Å):   {new_str}")
                self.system.ca_logger.warning(f"  Energy: {old_energy:.2f} → {new_energy:.2f} (ΔE={delta_energy:.2f}, bias={bias_energy:.2f})")
                self.system.ca_logger.warning(f"  Accepted: {accepted} (prob={acc_prob:.4f})")
...
class OutInAVBMCMove(Move):
    ...
    def attempt_move(self, anchor_idx):
        ...
        # Check new proximity after applying selected configuration
        if is_ca:
            has_close_ca_new, new_close_ca = self.system.check_ca_proximity(target_idx, 3.0)
        ...
        if is_ca and (has_close_ca_old or has_close_ca_new):
            if self.system.ca_logger:
                ...
                self.system.ca_logger.warning(f"Step {self.system.current_step} | OutInAVBMCMove | Ca {target_idx}")
                self.system.ca_logger.warning(f"  Initial Ca-Ca distances (Å): {old_str}")
                self.system.ca_logger.warning(f"  Final Ca-Ca distances (Å):   {new_str}")
                self.system.ca_logger.warning(f"  Energy: {old_energy:.2f} → {new_energy:.2f} (ΔE={delta_energy:.2f}, bias={bias_energy:.2f})")
                self.system.ca_logger.warning(f"  Accepted: {accepted} (prob={acc_prob:.4f})")
    ...
    def attempt_move(self, anchor_idx, Nin_idx):
        ...
        if is_ca and (has_close_ca_old or has_close_ca_new):
            if self.system.ca_logger:
                ...
                self.system.ca_logger.warning(f"Step {self.system.current_step} | NVTInOutMove | Ca {target_idx}")
                self.system.ca_logger.warning(f"  Initial Ca-Ca distances (Å): {old_str}")
                self.system.ca_logger.warning(f"  Final Ca-Ca distances (Å):   {new_str}")
                self.system.ca_logger.warning(f"  Energy: {old_energy:.2f} → {new_energy:.2f} (ΔE={delta_energy:.2f}, bias={bias_energy:.2f})")
                self.system.ca_logger.warning(f"  Accepted: {accepted} (prob={acc_prob:.4f})")
    ...
    def attempt_move(self, anchor_idx, Nin_idx):
        ...
        if is_ca and (has_close_ca_old or has_close_ca_new):
            if self.system.ca_logger:
                ...
                self.system.ca_logger.warning(f"Step {self.system.current_step} | NVTOutInMove | Ca {target_idx}")
                self.system.ca_logger.warning(f"  Initial Ca-Ca distances (Å): {old_str}")
                self.system.ca_logger.warning(f"  Final Ca-Ca distances (Å):   {new_str}")
                self.system.ca_logger.warning(f"  Energy: {old_energy:.2f} → {new_energy:.2f} (ΔE={delta_energy:.2f}, bias={bias_energy:.2f})")
                self.system.ca_logger.warning(f"  Accepted: {accepted} (prob={acc_prob:.4f})")
