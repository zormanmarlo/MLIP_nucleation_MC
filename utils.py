...
class PMFLammps:
    def __init__(self, types, box_size, model_path, table_path):
        '''Set up LAMMPS-based energy evaluator with allegro + tabulated prior (Buckingham + Wolf)'''
        from lammps import lammps
        ...
        self.table_inner_cutoff = 0.5  # Angstroms, inner cutoff of CaCO3_prior.table

    def _init_lammps(self):
        '''Initialize LAMMPS instance with allegro + tabulated prior pair style'''
        self.lmp = lammps()
        L = self.box_size
...
pair_style hybrid/overlay allegro table spline 4600
pair_coeff * * allegro {self.model_path} Ca C O
pair_coeff 1 1 table {self.table_path} CaCa 9.0
pair_coeff 1 2 table {self.table_path} CaC  9.0
pair_coeff 1 3 table {self.table_path} CaO  9.0
pair_coeff 2 2 table {self.table_path} CC   9.0
pair_coeff 2 3 table {self.table_path} CO   9.0
pair_coeff 3 3 table {self.table_path} OO   9.0
...
""")
    ...
    def energies(self, positions):
        '''Calculate total energy via LAMMPS; returns kcal/mol. Returns 1e10 for configs with atoms
        closer than the table inner cutoff (these would always be rejected by Metropolis anyway).'''
        if self.lmp is None:
            self._init_lammps()

        ordered = positions[self.lammps_order]
        diff = ordered[:, np.newaxis, :] - ordered[np.newaxis, :, :]
        diff -= self.box_size * np.round(diff / self.box_size)
        r2 = np.einsum('ijk,ijk->ij', diff, diff)
        np.fill_diagonal(r2, np.inf)
        if r2.min() < self.table_inner_cutoff ** 2:
            return 1e10

        lammps_pos = ordered.flatten()
